...
test_that("extract_expand_wildcard errors when all files missing", {
  expect_error(extract_expand_wildcard(files = c("non_valid_path.smk")), "Provided `files` do not exist")
})
...
test_that("extract_expand_wildcard warns when some files missing", {
  ...
  expect_warning(extract_expand_wildcard(files = c(valid_file, invalid_file)), regexp = "Following file do not exist: ")
})
...
test_that("extract_expand_wildcard warns on empty files", {
  ...
  expect_warning(extract_expand_wildcard(files = path), regexp = "Provided `files` are empty.")
})


test_that("extract_expand_wildcard warns when multiple wildcards found", {
  # character() included to create empty file
  path1 <- withr::local_tempfile(lines = c('expand("results/scater_plot/{sample}.png", sample=samples)'))
  path2 <- withr::local_tempfile(lines = c('expand("results/scater_plot/{sample}.png", sampler=samples)'))

  expect_warning(extract_expand_wildcard(files = c(path1, path2)), regexp = "Found more than one wildcard. Returning the first one")
})
...
test_that("extract_expand_wildcard warns when no wildcards present", {
  ...
  expect_warning(extract_expand_wildcard(files = c(path)), regexp = "No wildcard found. Returning empty character")
})
